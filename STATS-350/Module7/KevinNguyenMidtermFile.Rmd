...
plot(ppgdp_log, fertility_log, 
     ...
     col = "blue")
```
This model has a negative association, and it does make sense for the graph that we saw in the last part. Essentially, in part b, we saw a graph that showed us the y-values decreasing as x-increased. This scatter plot of the logarithms shows that same idea, but in a much clearer form, making this seem like a plausible model.
...
## Part d
- Calculate $\hat{y}_{i}$ (the log version), when p`pgdp = 1000`
- Calculate a 95% prediction interval for `log(fertility)`
...
```
The scatter plot seems to be positively associated and the linear regression line seems to align very well with the data points.


## Part c
- Do a significance test for beta1
- Then calculate r_squared (determination coefficient)
...
```
Using the $R^{2}$ value, About 3.59% of the height variation is explained by the model.
...
plot(mod1, which=1)
```
The residual plot is not a null plot
