...
d. $P(\bar{X}_40 > 351.4) = 0.005$:
We already have the mean and standard deviation for random variable sample mean. By the central limit theorem, since $n=40 \geq 30$, the distribution plotted by random variable sample mean is approximately normal. As a result, we can treat sampling distribution as normally distributed.
```r
# P(X_{40} > 351.4) = 1 - P(X_{40} <= 351.4)
...
# P(X <= 0.2825) = P(Rejecting H0 | H0 is false) = 0.947
pnorm(q=0.2825, mean=0.234, sd=0.03)
```

- **Extra Notes:** Assuming the null hypothesis, $\alpha$ encloses 0.04 area on the right tail, so the left side should have an area of 0.96. This results in $0.2825$. If the $\bar{X}_{n} \geq 0.2825$, then reject H0. Looking at our distribution with our true mean, let's find P(Type 2 Error) = P(Don't Reject H0 | mu=0.234). So we don't reject $H_{0}$ when $\bar{X}_{n} < 0.2825$. Again, we'll do calculations on the real distribution ($H_{1}$). But it's the idea that we're not rejecting $H_{0}$ on the $H_{1}$ distribution, which is akin to P(Don't reject H0 | H0 is actually false).
